...
class _MockClientClass(object):
  ...
  def upload_pipeline(self, pipeline_package_path):  # pylint: disable=invalid-name
    io_utils.copy_file(
...
class KubeflowHandlerTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    os.environ['KUBEFLOW_HOME'] = os.path.join(os.environ['HOME'],
                                               'kubeflow_pipelines')
  ...
  def test_get_handler_home(self):
    ...
    self.assertIn(handler._handler_home_dir, [
        os.environ[handler_home_dir],
